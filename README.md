...
- **Peningkatan Ketahanan & Penanganan Error Pembuatan Grup (Fix 502/ECONNREFUSED & findImpl error)**: Menambahkan `try-catch` wrapper di `GroupController` untuk memetakan error internal WhatsApp Web/Puppeteer ke dalam `HttpException` NestJS yang rapi. Mengintegrasikan `'error'` event listener pada client `whatsapp-web.js` untuk menangkap unhandled EventEmitter errors secara global dan mencegah proses backend Node.js mengalami crash total. Serta menambahkan opsi `{ autoSendInviteV4: false }` pada fungsi `client.createGroup` untuk melewati bug pemanggilan `Chat.find` internal WhatsApp Web (`this.findImpl is not a function`) saat terdapat kontak yang membatasi undangan grup pada setelan privasi mereka (HTTP 403).
